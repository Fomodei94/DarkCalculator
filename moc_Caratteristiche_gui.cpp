/****************************************************************************
** Meta object code from reading C++ file 'Caratteristiche_gui.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.10.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "View/Caratteristiche_gui.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'Caratteristiche_gui.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.10.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_Caratteristiche_gui_t {
    QByteArrayData data[11];
    char stringdata0[117];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Caratteristiche_gui_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Caratteristiche_gui_t qt_meta_stringdata_Caratteristiche_gui = {
    {
QT_MOC_LITERAL(0, 0, 19), // "Caratteristiche_gui"
QT_MOC_LITERAL(1, 20, 11), // "setVitalita"
QT_MOC_LITERAL(2, 32, 0), // ""
QT_MOC_LITERAL(3, 33, 1), // "i"
QT_MOC_LITERAL(4, 35, 10), // "setEnergia"
QT_MOC_LITERAL(5, 46, 9), // "setVigore"
QT_MOC_LITERAL(6, 56, 8), // "setForza"
QT_MOC_LITERAL(7, 65, 12), // "setDestrezza"
QT_MOC_LITERAL(8, 78, 15), // "setIntelligenza"
QT_MOC_LITERAL(9, 94, 7), // "setFede"
QT_MOC_LITERAL(10, 102, 14) // "refreshLivello"

    },
    "Caratteristiche_gui\0setVitalita\0\0i\0"
    "setEnergia\0setVigore\0setForza\0"
    "setDestrezza\0setIntelligenza\0setFede\0"
    "refreshLivello"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Caratteristiche_gui[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   54,    2, 0x0a /* Public */,
       4,    1,   57,    2, 0x0a /* Public */,
       5,    1,   60,    2, 0x0a /* Public */,
       6,    1,   63,    2, 0x0a /* Public */,
       7,    1,   66,    2, 0x0a /* Public */,
       8,    1,   69,    2, 0x0a /* Public */,
       9,    1,   72,    2, 0x0a /* Public */,
      10,    0,   75,    2, 0x0a /* Public */,

 // slots: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void,

       0        // eod
};

void Caratteristiche_gui::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Caratteristiche_gui *_t = static_cast<Caratteristiche_gui *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->setVitalita((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->setEnergia((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->setVigore((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: _t->setForza((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: _t->setDestrezza((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 5: _t->setIntelligenza((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 6: _t->setFede((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 7: _t->refreshLivello(); break;
        default: ;
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject Caratteristiche_gui::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_Caratteristiche_gui.data,
      qt_meta_data_Caratteristiche_gui,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *Caratteristiche_gui::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Caratteristiche_gui::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_Caratteristiche_gui.stringdata0))
        return static_cast<void*>(this);
    return QWidget::qt_metacast(_clname);
}

int Caratteristiche_gui::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
